#ifndef PARAMETRIZED_ADDER_H
#define PARAMETRIZED_ADDER_H

#include <systemc.h>
#include <cstdint>
#include <limits>

// Template-based parametrized adder with overflow/underflow detection
...
SC_MODULE(parametrized_adder)
{
    ...
    // Output ports
    sc_out<DataType>  sum;              // Result (saturated if overflow/underflow)
    ...
    // Constructor
    SC_CTOR(parametrized_adder)
    {
        SC_METHOD(compute_addition);
        sensitive << a << b;
    }
    
    // Process: combinational logic for addition with overflow/underflow detection
